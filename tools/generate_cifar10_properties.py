...
def _parse_args():
    parser = argparse.ArgumentParser(
        description="Generate properties for the CIFAR-10 networks."
    )
...
def main(args):
    ...
    for i, (idx, _, sx, target) in enumerate(data_loader):
        ...
        npy_img = sx[0].numpy().reshape((3,height,width))
        npy_img_path = os.path.join(args.output_dir, "%s.npy" % idx.item())
        np.save(npy_img_path, npy_img)
        ...
        if args.rm_transpose:
            with open(property_path, "w+") as property_file:
                property_file.write(
                    ...
                    "input_layer = 2\n" # START FROM 2(my cifar)/3(eran cifar)
