...
class TestDistillFullyConnectedNetwork(unittest.TestCase):
    def distill_and_check_accuracy(self, config_path):
        ...
        self.assertAlmostEqual(teacher_accuracy, 0.97, 2)
        self.assertLessEqual(round(abs(teacher_accuracy - student_accuracy), 2), 0.05)

    def test_2_same(self):
        self.distill_and_check_accuracy("tests/test_distill_fc/fc_to_same.toml")

    def test_fc_2_fc(self):
        self.distill_and_check_accuracy("tests/test_distill_fc/fc_to_fc.toml")
