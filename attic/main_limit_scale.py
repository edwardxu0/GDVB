...
def NAS_BS(args):
    ...
    '''
    # experiment to calculate nb neurons
    onnx_net = Onnxu("/p/d4v/dx3yy/3sub/conv_super.onnx")
    droppable_scalable_layers = get_layers(onnx_net)
    n = Network(CONFIGS)
    ...
    print([x.name for x in best_networks])
    ...
    pcents = [0.05, 0.1, 0.15, 0.2, 0.25]

    for lll in pcents:
        networks2 = []
        comb_trans_scale = []
        max_nb_layer_scale = CONFIGS['max_nb_layer_scale']
        max_nb_layer_scale = len(transformations_scale)
        #max_nb_layer_scale = lll

        for i in range(max_nb_layer_scale):
        ...
        print(np.sum(best_networks[args.tmp_id].nb_neurons))
        lb = np.sum(best_networks[args.tmp_id].nb_neurons)*(1-lll)
        ub = np.sum(best_networks[args.tmp_id].nb_neurons)*(1+lll)

        networks2 = [n for n in networks2 if np.sum(n.nb_neurons) > lb and np.sum(n.nb_neurons) < ub]

        print(lll)
        print(len(networks2))
        print(sorted([np.sum(n.nb_neurons) for n in networks2]))

    scale_network_size_epsilon = CONFIGS['scale_network_size']
    exit()
