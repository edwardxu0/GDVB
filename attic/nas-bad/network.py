...
class Network():
    ...
    def set_distillation_strategies(self, config, strategies):
    ...
    def distill(self):
        ...
        cmd = 'sbatch {}'.format(self.dis_slurm_path)
        os.system(cmd)
    ...
    def score(self, phase):
        ...
        if self.config['network_name'] == 'dave':
        ...
        if phase == 1:
            self.score = self.score_veri
        elif phase == 2:
            self.score = self.score_veri + self.score_ra
        else:
            assert False
